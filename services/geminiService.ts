  ...
  async analyzeChurchHealth(data: any) {
    ...
    try {
      ...
      if (error?.message?.includes('429') || error?.status === 429) {
        console.warn("⚠️ Gemini API: Quota excedida (429). Aguarde alguns instantes.");
        return "Insights temporariamente indisponíveis devido ao limite de uso da API gratuita.";
      }
