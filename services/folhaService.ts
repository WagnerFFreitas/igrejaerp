 ...
 * ------------------------
 * Serviço do frontend para folha_pagamento service.
 *
 * ONDE É USADO?
 * -------------
...
import { PayrollCalculation, PayrollInput, TaxConfig } from '../tipos';
 ...
 * ===============
 *
 * Define o bloco principal deste arquivo (folha_pagamento service).
 */
...
const generateMonthlyPayroll = (
  ...
  const dependentsCount =
    input.employee?.dependentes?.length ??
  ...
  const farmacia = toNumber(input.farmacia);
...
export const payrollService = {
  ...
  getPeriods: async (unitId?: string) => {
    return await apiClient.get('/folha_pagamento/periods', { unitId });
  },

  // Criar novo período
  createPeriod: async (periodData: any) => {
    return await apiClient.post('/folha_pagamento/periods', periodData);
  },

  // Salvar cálculo de folha
  saveCalculation: async (calculation: PayrollCalculation) => {
    return await apiClient.post('/folha_pagamento/calculations', calculation);
  },

  // Buscar cálculos de um período
  getCalculations: async (competencyMonth: string, unitId?: string) => {
    return await apiClient.get(`/folha_pagamento/calculations/${competencyMonth}`, { unitId });
  },
  ...
  getPayroll: async (unitId: string) => {
    ...
    return await apiClient.get(`/folha_pagamento/calculations/${competency}`, { unitId });
  },

  calculatePayroll: async (data: any) => {
    return await apiClient.post('/folha_pagamento/calculations', data);
  },
