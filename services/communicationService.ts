...
} from '../constants/communicationMock';
...
class CommunicationService {
  ...
  static async getEmailCampaigns(unitId: string): Promise<EmailCampaign[]> {
    try {
      ...
      if (campaigns.length === 0) {
        console.log('📊 Usando dados mock para campanhas de email');
        return MOCK_EMAIL_CAMPAIGNS.filter(c => c.unitId === unitId)
  ...
  static async getSMSMessages(unitId: string): Promise<SMSMessage[]> {
    try {
      ...
      if (messages.length === 0) {
        console.log('📊 Usando dados mock para mensagens SMS');
        return MOCK_SMS_MESSAGES.filter(m => m.unitId === unitId)
                      .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
      }
  ...
  static async getTemplates(unitId: string): Promise<CommunicationTemplate[]> {
    try {
      ...
      if (templates.length === 0) {
        console.log('📊 Usando dados mock para templates');
        return MOCK_TEMPLATES.filter(t => t.unitId === unitId)
  ...
  static async getGroups(unitId: string): Promise<CommunicationGroup[]> {
    try {
      ...
      // Se não houver dados reais, usar mock
      if (groups.length === 0) {
        console.log('📊 Usando dados mock para grupos');
        return MOCK_GROUPS.filter(g => g.unitId === unitId)
                      .sort((a, b) => a.name.localeCompare(b.name));
      }
