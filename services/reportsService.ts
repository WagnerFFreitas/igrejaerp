...
export const reportsService = {
  ...
  async gerarRelatorioContribuicoes(
    ...
    const contribuicoes = members
      ...
      .map(m => ({
        nome: m.name,
        departamento: m.mainMinistry || 'N/A',
  ...
  async gerarRelatorioPatrimonio(
    ...
    const inventario = assets.map(a => ({
      numeroPatrimonio: a.assetNumber,
      descricao: a.description,
      categoria: a.category,
      valorOriginal: `R$ ${a.acquisitionValue?.toFixed(2) || '0.00'}`,
      depreciacaoAcumulada: `R$ ${(a.accumulatedDepreciation || 0).toFixed(2)}`,
      valorLiquido: `R$ ${((a.acquisitionValue || 0) - (a.accumulatedDepreciation || 0)).toFixed(2)}`,
      status: a.status,
