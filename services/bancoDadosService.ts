...
const buildMemberProfileData = (member: any) => ({
  ...
  dependentes: Array.isArray(member.dependentes) ? member.dependentes : [],
...
const buildEmployeeProfileData = (employee: any) => ({
  birthDate: employee.birthDate || employee.data_nascimento || '',
  ...
  address: {
    ...
    complemento: employee.address?.complemento || '',
...
const mapEmployeeToApi = (employee: any) => ({
  ...
  id_unidade: normalizeUnitId(employee.id_unidade || employee.unitId),
  ...
  pis: employee.pis || '',
  data_nascimento: employee.data_nascimento || employee.birthDate || null,
  sexo: employee.sexo || employee.gender || null,
  estado_civil: employee.estado_civil || employee.maritalStatus || null,
  tipo_sanguineo: employee.tipo_sanguineo || employee.bloodType || '',
  email: employee.email || '',
  phone: employee.phone || '',
  celular: employee.celular || employee.phone || '',
  contato_emergencia: employee.contato_emergencia || employee.emergencyContact || '',
  ...
  address_complement: employee.address?.complemento || '',
  ...
  pais: employee.address?.country || 'Brasil',
  avatar: employee.avatar || null,
  dados_perfil: buildEmployeeProfileData(employee),
  esta_ativo: employee.esta_ativo ?? employee.status !== 'INACTIVE'
});

const mapTransactionToApi = (transaction: any) => ({
  id: transaction.id_transacao || transaction.id,
  id_unidade: normalizeUnitId(transaction.id_unidade || transaction.id_unidade || transaction.unitId),
...
export const dbService = {
  async getMembers(unitId?: string) {
      const response = await apiClient.get<any>('/membros', {
  ...
  async updateMember(member: any) {
    ...
    const saved = await apiClient.put<any>(`/membros/${member.id}`, mapMemberToApi(member));
    return mapMemberFromApi(saved);
  },

  async deleteMember(id: string) {
    return apiClient.delete(`/membros/${id}`);
  },

  async getTransactions(unitId?: string) {
    return apiClient.get<any[]>('/transacoes', {
      unitId: normalizeUnitId(unitId),
      limit: 500
    });
  },
  ...
  async saveTransaction(transaction: any) {
    ...
    if (isPersistedId(transaction?.id)) {
      const saved = await apiClient.put<any>(`/transacoes/${transaction.id}`, payload);
      return saved.id;
    }
    const saved = await apiClient.post<any>('/transacoes', payload);
    return saved.id;
  },

  async getAccounts(unitId?: string) {
    return apiClient.get<any[]>('/contas-bancarias', { unitId: normalizeUnitId(unitId) });
  },

  async saveAccount(account: any) {
    const payload = {
      unitId: account.unitId || account.id_unidade,
      name: account.name,
    ...
    if (isPersistedId(account?.id)) {
      const saved = await apiClient.put<any>(`/contas-bancarias/${account.id}`, payload);
      return saved.id;
    }
    const saved = await apiClient.post<any>('/contas-bancarias', payload);
    return saved.id;
  },
  ...
  async getEmployees(unitId?: string) {
    return apiClient.get<any[]>('/funcionarios', { unitId: normalizeUnitId(unitId) });
  },

  async saveEmployee(employee: any) {
    const payload = mapEmployeeToApi(employee);
    const saved = isPersistedId(employee?.id)
      ? await apiClient.put<any>(`/funcionarios/${employee.id}`, payload)
      : await apiClient.post<any>('/funcionarios', payload);
    return saved.id;
  },
  ...
  async getLeaves(unitId?: string) {
    return apiClient.get<any[]>('/afastamentos', { unitId: normalizeUnitId(unitId) });
  },

  async saveLeave(leave: any, isEditing?: boolean) {
    if (isEditing && isPersistedId(leave.id)) {
      return apiClient.put(`/afastamentos/${leave.id}`, leave);
    }
    return apiClient.post('/afastamentos', leave);
  },

  async deleteLeave(id: string) {
    return apiClient.delete(`/afastamentos/${id}`);
  },

  async getUnits() {
    const response = await apiClient.get<{unidades: any[]}>('/unidades');
    return (response.unidades || []).map(mapUnitFromApi);
  },

  async getEvents(unitId?: string) {
    return apiClient.get<any[]>('/eventos', { unitId: normalizeUnitId(unitId) });
  }
};
...
export const InicializadorDados = {
