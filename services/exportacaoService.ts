...
export const exportService = {
  ...
  async exportarHoleritesPDF(payrolls: any[]): Promise<void> {
    ...
    try {
      ...
      payrolls.forEach((folha_pagamento, index) => {
        if (index > 0) {
          doc.addPage();
        }

        const unit = folha_pagamento.unit;
        const month = folha_pagamento.month || '00';
        const year = folha_pagamento.year || '0000';
        ...
        const drawHolerite = (yOffset: number, title: string) => {
          ...
          doc.setFontSize(8);
          doc.setFont('helvetica', 'bold');
          doc.text(String(folha_pagamento.matricula || folha_pagamento.id?.substring(0, 7) || '-'), margin + 2, startY + 28);
          doc.text(String(folha_pagamento.employeeName || folha_pagamento.nome || 'NÃO INFORMADO').toUpperCase(), margin + 22, startY + 28);
          doc.text(String(folha_pagamento.cbo || '-'), margin + 112, startY + 28);
          doc.text(String(folha_pagamento.cargo || '-').toUpperCase(), margin + 142, startY + 28);
          ...
          // Itens
          addRow('001', 'SALÁRIO BASE MENSAL', '30D', folha_pagamento.salario_base || 0, 0);
          
          if (folha_pagamento.ats_percentual > 0) {
            const atsValue = (folha_pagamento.salario_base || 0) * (folha_pagamento.ats_percentual / 100);
            addRow('010', 'ADIC. TEMPO SERVIÇO (ATS)', `${folha_pagamento.ats_percentual}%`, atsValue, 0);
          }

          if (folha_pagamento.auxilio_moradia > 0) {
            addRow('080', 'AUXÍLIO MORADIA / PREBENDA', 'FIXO', folha_pagamento.auxilio_moradia, 0);
          }

          if (folha_pagamento.he50_qtd > 0) {
            const valorHora = (folha_pagamento.salario_base || 0) / 220;
            const he50Valor = valorHora * 1.5 * folha_pagamento.he50_qtd;
            addRow('011', 'HORAS EXTRAS 50%', `${folha_pagamento.he50_qtd}H`, he50Valor, 0);
          }

          if (folha_pagamento.he100_qtd > 0) {
            const valorHora = (folha_pagamento.salario_base || 0) / 220;
            const he100Valor = valorHora * 2.0 * folha_pagamento.he100_qtd;
            addRow('012', 'HORAS EXTRAS 100%', `${folha_pagamento.he100_qtd}H`, he100Valor, 0);
          }

          if (folha_pagamento.comissoes > 0) {
            addRow('020', 'COMISSÕES', 'VAR', folha_pagamento.comissoes, 0);
          }

          if (folha_pagamento.gratificacoes > 0) {
            addRow('021', 'GRATIFICAÇÕES', 'VAR', folha_pagamento.gratificacoes, 0);
          }

          if (folha_pagamento.salario_familia > 0) {
            addRow('030', 'SALÁRIO FAMÍLIA', 'DEP', folha_pagamento.salario_familia, 0);
          }

          // Descontos
          if (folha_pagamento.inss > 0) {
            addRow('901', 'INSS - PREVIDÊNCIA SOCIAL', 'VAR', 0, folha_pagamento.inss);
          }

          if (folha_pagamento.irrf > 0) {
            addRow('902', 'IRRF - IMPOSTO DE RENDA', 'VAR', 0, folha_pagamento.irrf);
          }

          if (folha_pagamento.vale_transporte_total > 0 && folha_pagamento.vt_ativo) {
            addRow('910', 'VALE TRANSPORTE', '6%', 0, folha_pagamento.vale_transporte_total);
          }

          if (folha_pagamento.vale_alimentacao > 0 && folha_pagamento.va_ativo) {
            addRow('970', 'DESCONTO VALE ALIMENTAÇÃO', 'FIXO', 0, folha_pagamento.vale_alimentacao);
          }

          if (folha_pagamento.vale_refeicao > 0 && folha_pagamento.vr_ativo) {
            addRow('971', 'DESCONTO VALE REFEIÇÃO', 'FIXO', 0, folha_pagamento.vale_refeicao);
          }

          if (folha_pagamento.plano_saude_colaborador > 0 && folha_pagamento.ps_ativo) {
            addRow('950', 'PLANO DE SAÚDE (TITULAR)', 'PARC', 0, folha_pagamento.plano_saude_colaborador);
          }

          if (folha_pagamento.plano_odontologico > 0 && folha_pagamento.po_ativo) {
            addRow('951', 'PLANO ODONTOLÓGICO', 'PARC', 0, folha_pagamento.plano_odontologico);
          }

          if (folha_pagamento.adiantamento > 0) {
            addRow('980', 'ADIANTAMENTO SALARIAL', 'VAR', 0, folha_pagamento.adiantamento);
          }

          if (folha_pagamento.pensao_alimenticia > 0) {
            addRow('981', 'PENSÃO ALIMENTÍCIA', 'JUDIC', 0, folha_pagamento.pensao_alimenticia);
          }
          ...
          const finalProventos = folha_pagamento.total_proventos || calcTotalProventos;
          const finalDescontos = folha_pagamento.total_descontos || calcTotalDescontos;
          const finalLiquido = folha_pagamento.salario_liquido || (finalProventos - finalDescontos);
          ...
          doc.setFontSize(5);
          doc.setFont('helvetica', 'normal');
          doc.text('CERTIFICAÇÃO DIGITAL ADJPA ERP + PROTOCOLO DE AUTENTICIDADE: ' + String(folha_pagamento.id || 'LOCAL').toUpperCase(), margin + 5, startY + 115);
