 ...
 * ===============
 *
 * Define o bloco principal deste arquivo (types).
 */
...
export interface PayrollPeriod {
  ...
  ano?: number;
  year?: number;
  situacao?: 'OPEN' | 'CLOSED' | 'PROCESSING';
...
export interface Asset {
  ...
  valorAtual: number;              // Valor atualizado
  currentValue?: number;
  ...
  localizacao: string;                  // Localização atual (sala, departamento)
  location?: string;
  responsavel?: string;              // Responsável pelo bem
  responsible?: string;
  situacao: AssetStatus;               // Status atual
  status?: AssetStatus;
  photos?: string[];                 // URLs das fotos
  documents?: string[];              // URLs de documentos (NF, contrato)
  vidaUtilMeses: number;          // Vida útil em meses
  usefulLifeMonths?: number;
  taxaDepreciacao: number;          // Taxa de depreciação anual (%)
  depreciationRate?: number;
  metodoDepreciacao: DepreciationMethod;
  depreciationMethod?: DepreciationMethod;
  valorContabilAtual: number;          // Valor contábil atual
  currentBookValue?: number;
  depreciacaoAcumulada: number;   // Depreciação acumulada
  accumulatedDepreciation?: number;
  valorResidual?: number;            // Valor residual (opcional)
  residualValue?: number;
  dataUltimoInventario?: string;        // Última conferência de inventário
  lastInventoryDate?: string;
  contagemInventario?: number;           // Quantidade no último inventário
  inventoryCount?: number;
  numeroPatrimonio: string;               // Número de patrimônio
  assetNumber?: string;
...
export interface ChurchEvent {
  ...
  tipo?: 'SERVICE' | 'MEETING' | 'EVENT';
  type?: 'SERVICE' | 'MEETING' | 'EVENT';
  // Campos para escala de voluntários
  escalaVoluntarios?: VolunteerSchedule[];
  volunteerSchedule?: VolunteerSchedule[];
  // Campos para recorrência de eventos
  ehRecorrente?: boolean;
  isRecurring?: boolean;
  padraoRecorrencia?: 'NONE' | 'WEEKLY' | 'MONTHLY';
  recurrencePattern?: 'NONE' | 'WEEKLY' | 'MONTHLY';
  dataFimRecorrencia?: string;
  recurrenceEndDate?: string;
  eventoPaiId?: string;
  parentEventId?: string;
  ehEventoGerado?: boolean;
  isGeneratedEvent?: boolean;
}

export interface VolunteerSchedule {
  id: string;
  ministerio?: string;             // Ministério (Louvor, Ensino, Ação Social, etc.)
  ministry?: string;
  funcao?: string;                 // Função dentro do evento (Líder, Músico, Professor, etc.)
  role?: string;
...
export * from './types/contabil';
export * from './types/financeiro';

// =====================================================
// ALIASES PARA COMPATIBILIDADE (Inglês → Português)
// =====================================================
// Mantém os nomes antigos em inglês como aliases para os novos nomes em português

export type Payroll = PayrollPeriod | PayrollCalculation;
export type UserAuth = Usuario;
export type Unit = Unidade;
export type Transaction = Transacao;
export type Member = Membro;
export type Employee = Funcionario;
export type LGPDConsentAlias = LGPDConsent;
