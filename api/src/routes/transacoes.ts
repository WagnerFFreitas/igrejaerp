...
const normalizarTransacao = (payload: any): Record<string, any> => ({
  ...
  id_fornecedor:     payload.id_fornecedor     || payload.idFornecedor     || payload.supplierId || null,
  ...
  nome_fornecedor:   payload.nome_fornecedor   || payload.providerName     || payload.supplierName || null,
...
const mapearTransacao = (row: any) => ({
  ...
  idFornecedor:    row.id_fornecedor || null,
  ...
  nomeFornecedor:  row.nome_fornecedor || row.fornecedor_nome || null,
...
router.get('/', async (req, res) => {
  try {
    ...
    if (idUnidade)  { where += ` AND t.id_unidade = $${i++}`;  params.push(idUnidade); }
    if (tipo)       { where += ` AND t.tipo = $${i++}`;         params.push(tipo); }
    if (situacao)   { where += ` AND t.situacao = $${i++}`;     params.push(situacao); }
    ...
    const result = await db.query(
      `SELECT t.*, f.nome AS fornecedor_nome
       FROM transacoes t
       LEFT JOIN fornecedores f ON f.id_fornecedor = t.id_fornecedor
       ${where} ORDER BY t.data_transacao DESC LIMIT $${i++} OFFSET $${i++}`,
      params
    );
