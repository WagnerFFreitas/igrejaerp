...
import { APP_PERMISSION_MODULES, getEffectivePermissions, replaceUserPermissions } from '../services/permissoes-servico';
import { AuthenticatedRequest, requireAuth } from '../middleware/autenticacao';

/**
 * BLOCO PRINCIPAL
 * ===============
 *
 * Define o bloco principal deste arquivo (usuarios).
 */
...
async function ensureCanManageUsers(req: AuthenticatedRequest, res: any): Promise<boolean> {
  ...
  const canManage =
    permissions.some(permission => permission.moduleCode === 'usuarios' && permission.canManage) ||
