...
class TesourariaService {
  ...
  // ─── TREASURY ACCOUNTS (usa contas_financeiras via /contas-bancarias) ────────────

  static async getTreasuryAccounts(unitId: string): Promise<TreasuryAccount[]> {
    try {
      const data = await apiClient.get<any[]>('/contas-bancarias', { unitId });
  ...
  static async saveTreasuryAccount(account: TreasuryAccount): Promise<void> {
    // TreasuryAccount mapeia para contas_financeiras
    const payload = {
    ...
    try {
      if (account.id && !account.id.startsWith('tmp-')) {
        await apiClient.put(`/contas-bancarias/${account.id}`, payload);
      } else {
        await apiClient.post('/contas-bancarias', payload);
      }
    } catch (e) {
  ...
  static async generateTreasuryAlerts(unitId: string): Promise<TreasuryAlert[]> {
    ...
    const contas_bancarias = await this.getTreasuryAccounts(unitId);
  ...
  static async generateFinancialPosition(unitId: string): Promise<FinancialPosition> {
    ...
    const disponibilidades = contas_bancarias.reduce((s, a) => s + a.saldoDisponivel, 0);
  ...
  static async getCurrentBalance(unitId: string): Promise<number> {
    const contas_bancarias = await this.getTreasuryAccounts(unitId);
    return contas_bancarias.reduce((s, a) => s + a.saldoDisponivel, 0);
  }

  static async getTreasurySummary(unitId: string) {
    const [contas_bancarias, investments, loans, alerts] = await Promise.all([
    ...
    const saldoTotal        = contas_bancarias.reduce((s, a) => s + a.saldoDisponivel, 0);
