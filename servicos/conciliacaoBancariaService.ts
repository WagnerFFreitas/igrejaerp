...
class ConciliacaoBancariaService {
  ...
  static async getReconciliations(unitId?: string) {
    try {
      const data = await apiClient.get<any[]>('/conciliacoes-bancarias', { unitId });
      return data || [];
    } catch (e) {
    ...
    try {
      ...
      for (const rec of reconciliations) {
        if (rec.bank_account_id === accountId || rec.bankAccountId === accountId) {
          const txs = await apiClient.get<any[]>(`/conciliacoes-bancarias/${rec.id}/transacoes`);
          results.push(...(txs || []));
        }
      }
      return results;
  ...
  static async importBankStatement(accountId: string, file: File, unitId: string) {
    try {
      ...
      const reconciliation = await apiClient.post<any>('/conciliacoes-bancarias', {
        unitId,
      ...
      const imported = await apiClient.post<any>(`/conciliacoes-bancarias/${reconciliation.id}/transacoes`, {
        unitId,
        bankAccountId: accountId,
        dataTransacao: today,
  ...
  static async updateReconciliation(id: string, data: any) {
    try {
      return await apiClient.put<any>(`/conciliacoes-bancarias/${id}`, data);
    } catch (e) {
  ...
  static async matchTransaction(reconciliationId: string, bankTxId: string, systemTransactionId: string) {
    try {
      return await apiClient.patch<any>(
        `/conciliacoes-bancarias/${reconciliationId}/transacoes/${bankTxId}/match`,
