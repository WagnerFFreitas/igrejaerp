...
import { withTimeout } from '../utils/promessaUtils';
...
export class UsuarioService {
  ...
  static async getUsers(): Promise<Usuario[]> {
    try {
      return await withTimeout(IndexedDBService.getAll(UsuarioService.USERS_STORE), 10000);
    } catch (error) {
  ...
  static async saveUser(user: Usuario): Promise<void> {
    try {
      await withTimeout(IndexedDBService.save(UsuarioService.USERS_STORE, user), 10000);
    } catch (error) {
      console.error('❌ UsuarioService: Erro ao salvar usuário:', error);
      throw error;
    }
  }
