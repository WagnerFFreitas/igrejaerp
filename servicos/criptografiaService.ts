...
export class CriptografiaService {
  ...
  static maskBankData(bank: string, agencia: string, account: string): {
    bank?: string;
    agencia?: string;
    account?: string;
  } {
    return {
      ...
      agencia: agencia ? agencia.replace(/(\d{3})-(\d)/, '$1-*') : undefined,
  ...
  static sanitizeEmployee(employee: any): any {
    ...
    return {
      ...
      esta_ativo: employee.esta_ativo
    };
  }
  ...
  static sanitizePayroll(folha_pagamento: any): any {
    return {
      ...folha_pagamento,
      cpf: this.maskCPF(folha_pagamento.cpf),
      rg: this.maskRG(folha_pagamento.rg),
      pis: this.maskPIS(folha_pagamento.pis),
      ctps: this.maskCTPS(folha_pagamento.ctps),
      email: this.maskEmail(folha_pagamento.email),
      phone: this.maskPhone(folha_pagamento.phone),
      // Salário e valores financeiros mascarados
      salario_base: this.maskMonetaryValue(folha_pagamento.salario_base),
      comissoes: this.maskMonetaryValue(folha_pagamento.comissoes),
      gratificacoes: this.maskMonetaryValue(folha_pagamento.gratificacoes),
      premios: this.maskMonetaryValue(folha_pagamento.premios),
      auxilio_moradia: this.maskMonetaryValue(folha_pagamento.auxilio_moradia),
      salario_familia: this.maskMonetaryValue(folha_pagamento.salario_familia),
      // Mantém dados não sensíveis
      id: folha_pagamento.id,
      idUnidade: folha_pagamento.idUnidade,
      employeeName: folha_pagamento.employeeName,
      matricula: folha_pagamento.matricula,
      cargo: folha_pagamento.cargo,
      departamento: folha_pagamento.departamento,
      data_admissao: folha_pagamento.data_admissao
    };
  }
...
export default CriptografiaService;
