...
import { analyticsService } from './analiticaService';
import { exportService } from './exportacaoService';

export const reportsService = {
  // TODO: Implementar acesso ao PostgreSQL
  // db: getFirestore(),
  ...
  async gerarRelatorioPatrimonio(
    ...
    const patrimonios: Asset[] = [];
    snapshot.forEach(doc => {
      patrimonios.push({ id: doc.id, ...doc.data() } as Asset);
    });

    const inventario = patrimonios.map(a => ({
