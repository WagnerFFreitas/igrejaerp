// Script final para recriar os funcionários que faltam
// Execute no console após fazer login

const recriarFuncionariosFaltantes = async () => {
  try {
    console.log('🔄 Recriando funcionários que faltam...');
    
    // Funcionários para criar (com IDs únicos)
    const funcionariosFaltantes = [
      {
        id: 'emp_' + Date.now() + '_001',
        unitId: 'u-sede',
    ...
    for (const funcionario of funcionariosFaltantes) {
      await dbService.saveEmployee(funcionario);
      console.log(`✅ Funcionário criado: ${funcionario.employeeName}`);
    }
    ...
    // Verificar resultado
    setTimeout(async () => {
      const todosFuncionarios = await dbService.getEmployees('u-sede');
      console.log(`📊 Total de funcionários agora: ${todosFuncionarios.length}`);
      
      console.log('\n📋 Lista completa:');
      todosFuncionarios.forEach((emp, index) => {
        console.log(`${index + 1}. ${emp.employeeName || emp.name}`);
      });
    }, 1000);
...
window.recriarFuncionariosFaltantes = recriarFuncionariosFaltantes;

console.log('📋 Script de recriação carregado!');
console.log('🔄 Para executar, digite: recriarFuncionariosFaltantes()');
