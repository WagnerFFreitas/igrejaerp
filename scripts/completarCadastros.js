// Script para COMPLETAR os cadastros existentes (Wagner e Jefferson)
          ...
          if (btnNovo) {
            ...
            setTimeout(() => {
              ...
              const preencherCampo = (seletor, valor) => {
                ...
                if (campo) {
                  campo.value = valor;
                  // Disparar evento change para React reconhecer
              ...
              const clicarAba = (texto) => {
                const abas = document.querySelectorAll('button, div[role="tab"], [role="tab"]');
              ...
              // 2. DADOS CONTRATUAIS
              console.log('📋 Preenchendo Dados Contratuais...');
              clicarAba('Contrato');
              setTimeout(() => {
              ...
              // 3. BANCO E HORAS
              console.log('📋 Preenchendo Banco e Horas...');
              clicarAba('Banco');
              setTimeout(() => {
              ...
              // 4. BENEFÍCIOS
              console.log('📋 Preenchendo Benefícios...');
              clicarAba('Benefícios');
              setTimeout(() => {
                // VT
                const vtAtivo = document.querySelector('input[name*="vt_ativo"]');
                if (vtAtivo && dados.vale_transporte_total > 0) {
                  vtAtivo.checked = true;
                  preencherCampo('input[name*="vale_transporte"]', dados.vale_transporte_total?.toString() || '');
                }
                
                // VA
                const vaAtivo = document.querySelector('input[name*="va_ativo"]');
                if (vaAtivo && dados.vale_alimentacao > 0) {
                  vaAtivo.checked = true;
                  preencherCampo('input[name*="vale_alimentacao"]', dados.vale_alimentacao?.toString() || '');
                }
                
                // VR
                const vrAtivo = document.querySelector('input[name*="vr_ativo"]');
                if (vrAtivo && dados.vale_refeicao > 0) {
                  vrAtivo.checked = true;
                  preencherCampo('input[name*="vale_refeicao"]', dados.vale_refeicao?.toString() || '');
                }
                
                // Plano Saúde
                const psAtivo = document.querySelector('input[name*="ps_ativo"]');
                if (psAtivo && dados.plano_saude_colaborador > 0) {
                  psAtivo.checked = true;
              ...
              console.log('✅ Formulário COMPLETO preenchido para:', dados.employeeName);
...
// Criar funções para completar cadastros
window.completarWagner = () => preencherFormularioCompleto(dadosWagner);
window.completarJefferson = () => preencherFormularioCompleto(dadosJefferson);

console.log('✅ Script de cadastros completos carregado!');
console.log('📋 Para completar os cadastros:');
console.log('   - Wagner: completarWagner()');
console.log('   - Jefferson: completarJefferson()');
console.log('📋 Execute um por vez e aguarde TODAS as abas serem preenchidas!');
console.log('🎯 Isso vai preencher DADOS PESSOAIS, CONTRATUAIS, BANCO e BENEFÍCIOS!');
