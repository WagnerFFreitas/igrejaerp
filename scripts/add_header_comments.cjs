...
const ignoredDirs = ['node_modules', '.git', 'dist', 'build', '.vscode'];
...
function insertCommentIntoFile(filePath) {
  ...
  const exportLineIndex = lines.findIndex(line => /^(export\s+default|export\s+const|export\s+function|export\s+class|export\s+interface|export\s+type|const\s+|function\s+|class\s+)/.test(line.trim()));
