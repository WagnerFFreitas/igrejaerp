/**
 * ============================================================================
 * TEMPLATECRACHAFUNCIONARIO.TSX
 * ============================================================================
 *
 * O QUE ESTE ARQUIVO FAZ?
 * ------------------------
 * Componente visual do frontend para template cracha funcionario.
 *
 * ONDE É USADO?
 * -------------
 ...
 * ===============
 *
 * Define o bloco principal deste arquivo (template cracha funcionario).
 */
...
export const TemplateCrachaFuncionario: React.FC<TemplateCrachaFuncionarioProps> = ({ employee, unit, id }) => {
  ...
  const admissaoFormatada = (() => {
    const d = employee.data_admissao || (employee as any).admissionDate;
    if (!d) return '—';
    try { return new Date(d + 'T00:00:00').toLocaleDateString('pt-BR'); }
    catch { return '—'; }
  })();

  // Telefone real da unidade
  const telefoneUnidade = unit?.telefone || '(21) 2675-7036';
    ...
    >
      ...
      <div className="relative z-20 flex flex-col items-center w-full mb-3 gap-0">
        ...
        <div className="text-center flex flex-col gap-[0.2mm] w-full -mt-2">
          <p className="text-[7.5px] font-black text-[#003399] leading-normal uppercase tracking-tighter whitespace-nowrap">{unit?.nome || 'CARREGANDO...'}</p>
          <p className="text-[5.5px] font-bold text-slate-500 uppercase leading-none">{`CNPJ ${unit?.cnpj || '00.000.000/0000-01'}`}</p>
          <p className="text-[5.5px] font-black text-[#003399] uppercase leading-none">{`TEL.: ${telefoneUnidade}`}</p>
    ...
    >
      ...
      <div className="relative z-10 space-y-1.5 mb-2">
        ...
        <div className="flex justify-between pb-0.5">
          ...
          <p className="text-[8px] font-bold text-slate-800">{admissaoFormatada}</p>
        </div>
        <div className="flex justify-between pb-0.5">
