...
export default function AvaliacaoDesempenho({ 
  ...
  useEffect(() => {
    ...
    const mockEvaluations: PerformanceEvaluation[] = [
      {
        ...
        evaluatorId: user?.id || 'manager-001',
        evaluatorName: user?.name || 'Gestor',
        ...
        overallScore: 85,
        overallRating: 'GOOD',
        strengths: ['Liderança eficaz', 'Boa comunicação', 'Proatividade'],
        improvementAreas: ['Gestão de tempo', 'Delegação de tarefas'],
        comments: 'Bom desempenho geral com oportunidades de melhoria na gestão de tempo.',
        competencies: [
          {
            id: 'comp-001',
            competencyId: 'lead-001',
            ...
            score: 88,
            level: 'ADVANCED',
            evidence: ['Liderou projeto X com sucesso', 'Mentoria de 3 novos membros'],
        ...
        goals: [
          {
            ...
            description: 'Meta de produtividade Q1',
            category: 'PRODUCTIVITY',
            targetValue: '100 unidades',
            actualValue: '95 unidades',
            achievementPercentage: 95,
            status: 'COMPLETED',
            dueDate: '2024-03-31',
            comments: 'Meta quase alcançada'
          }
        ],
        pdiPlan: [
          {
            id: 'pdi-001',
            area: 'Desenvolvimento de Liderança',
            action: 'Curso de Gestão de Tempo',
            responsible: 'EMPLOYEE',
            priority: 'HIGH',
            deadline: '2024-04-30',
            status: 'IN_PROGRESS',
            resources: ['Plataforma de cursos online'],
            progress: 60,
            notes: 'Em andamento, conclusão prevista para abril'
          }
        ],
        ...
        approvedBy: 'hr-001',
        approvedDate: '2024-04-01T15:00:00Z'
      }
    ];
    ...
    const mockCycles: EvaluationCycle[] = [
      {
        id: 'cycle-001',
        name: 'Ciclo de Avaliação Q1/2024',
        description: 'Avaliação trimestral do primeiro quadrimestre',
        cycleType: 'QUARTERLY',
        ...
        subordinateEvaluation: false,
        clientEvaluation: false,
        competencyTemplates: ['comp-001', 'comp-002'],
        goalTemplates: ['goal-001'],
        totalEmployees: 25,
  ...
  const filteredEvaluations = evaluations.filter(eval => {
    const matchesSearch = eval.employeeName.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         eval.evaluatorName.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const reportData = {
        ...
        averageScore: evaluations.reduce((sum, eval) => sum + eval.overallScore, 0) / evaluations.length,
        evaluationsByRating: {
