...
import UsersService, { AppPermission, AppUser, PermissionModule } from '../src/services/usuariosService';
...
export const UserPermissionsPanel: React.FC<UserPermissionsPanelProps> = ({ currentUser }) => {
  const [usuarios, setUsers] = useState<AppUser[]>([]);
  const [modules, setModules] = useState<PermissionModule[]>([]);
  const [unidades, setUnits] = useState<Array<{ id: string; name: string }>>([]);
  ...
  const selectedUser = useMemo(
    () => usuarios.find(user => user.id === selectedUserId) || null,
