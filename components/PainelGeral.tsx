...
import { 
  Users, TrendingUp, DollarSign, Target, Sparkles, AlertTriangle, RotateCcw, Clock, Cake,
  BarChart3, PieChart, Activity, Calendar, Mail, MessageSquare, TrendingDown, Award,
  Brain, Zap, RefreshCw, Download, Settings, Car, AlertCircle
} from 'lucide-react';
...
export const PainelGeral: React.FC<PainelGeralProps> = ({ user, members, employees }) => {
  ...
  const fetchInsights = useCallback(async (force = false) => {
    ...
    try {
      ...
      const totalRevenue = 25000; // Simulação - poderia vir de dados reais
      const totalExpenses = 18000; // Simulação
  ...
  const generateDetailedInsights = async () => {
    ...
    try {
      ...
      **Forneça:**
      ...
      // Usar o método direto para análise mais detalhada
      ...
      if (data.candidates && data.candidates[0] && data.candidates[0].content) {
        ...
        setDetailedInsights('Não foi possível gerar a análise detalhada. Tente novamente.');
      }
    } catch (error) {
  ...
  const generatePredictions = async () => {
    ...
    try {
      ...
      **Projeções solicitadas:**
      ...
      if (data.candidates && data.candidates[0] && data.candidates[0].content) {
        setPredictions(data.candidates[0].content.parts[0].text);
      } else {
        setPredictions('Não foi possível gerar as projeções. Tente novamente.');
      }
    } catch (error) {
            ...
            {[
              ...
              <div key={i} className="bg-white p-6 rounded-[1.5rem] border border-slate-100 flex flex-col shadow-sm hover:shadow-md transition-shadow">
                <div className="flex justify-between mb-3">
                  ...
                  <span className={`text-sm font-black ${s.t.startsWith('+') ? 'text-emerald-600 bg-emerald-50' : 'text-rose-600 bg-rose-50'} px-3 py-1 rounded-full`}>{s.t}</span>
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
            <div className="lg:col-span-8 bg-white p-8 rounded-[2rem] border border-slate-100 h-96 shadow-sm">
              <h3 className="text-base font-black text-slate-900 uppercase tracking-widest mb-6 border-b pb-2 flex items-center gap-2">
                <TrendingUp size={18} className="text-indigo-600"/> Evolução da Arrecadação
              </h3>
              <ResponsiveContainer width="100%" height="70%">
                <AreaChart data={chartData}>
                  ...
                  <YAxis tick={{fontSize: 12, fontWeight: 700}} axisLine={false} />
                  <Tooltip />
