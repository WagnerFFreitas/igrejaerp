...
import { 
  ...
  Plus, BarChart3, AlertCircle, Clock, Settings, Play
} from 'lucide-react';
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const mockReconciliations: BankReconciliation[] = [
          {
            ...
            saldoInicial: 10000,
            saldoFinal: 15000,
            saldoConciliado: 14500,
            diferenca: 500,
            status: 'DISCREPANCY',
            percentualConciliacao: 96.7,
            totalTransacoesBanco: 45,
            totalTransacoesSistema: 42,
            transacoesConciliadas: 40,
            transacoesNaoConciliadas: 7,
            divergencias: [],
            conciliadoPor: 'user-001',
            conciliadoPorNome: 'João Financeiro',
            dataConciliacao: '2024-04-01T10:00:00Z',
            observacoes: 'Conciliação mensal com algumas divergências',
            anexos: [],
        ...
        const mockTransactions: BankTransaction[] = [
          {
            ...
            statusConciliacao: 'CONCILIATED',
            transacaoSistemaId: 'sys-001',
            dataConciliacao: '2024-04-01T10:00:00Z',
        ...
        const mockDiscrepancies: BankDiscrepancy[] = [
          {
            id: 'disc-001',
            reconciliationId: 'rec-001',
            tipo: 'MISSING_IN_SYSTEM',
            gravidade: 'MEDIUM',
            transacaoBancoId: 'tx-003',
            descricao: 'Transação bancária não encontrada no sistema',
            valorDiferenca: 500,
            dataEsperada: '2024-03-20',
            status: 'OPEN',
      ...
      <div className="flex items-center gap-4 mb-6">
        <div className="flex-1">
          ...
          >
            ...
            <option value="acc-001">Conta Principal - Banco do Brasil</option>
            <option value="acc-002">Conta Secundária - Itaú</option>
          </select>
        </div>
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="text-lg font-semibold text-slate-900">{reconciliation.bankAccountName}</h3>
                    ...
                    <div className="flex items-center gap-4 text-sm text-slate-600 mb-3">
                      <div className="flex items-center gap-1">
                        <Calendar size={14} />
                        <span>{new Date(reconciliation.dataInicio).toLocaleDateString('pt-BR')} - {new Date(reconciliation.dataFim).toLocaleDateString('pt-BR')}</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <User size={14} />
                        <span>{reconciliation.conciliadoPorNome}</span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                      <div className="bg-slate-50 p-3 rounded-lg">
                        ...
                        <p className="text-lg font-bold text-slate-900">{formatCurrency(reconciliation.saldoInicial)}</p>
                      </div>
                      <div className="bg-slate-50 p-3 rounded-lg">
                        <p className="text-xs text-slate-500 mb-1">Saldo Final</p>
                        <p className="text-lg font-bold text-slate-900">{formatCurrency(reconciliation.saldoFinal)}</p>
                      </div>
                      <div className="bg-blue-50 p-3 rounded-lg">
                        <p className="text-xs text-blue-600 mb-1">Conciliado</p>
                        <p className="text-lg font-bold text-blue-600">{formatCurrency(reconciliation.saldoConciliado)}</p>
                      </div>
                      <div className={`${reconciliation.diferenca === 0 ? 'bg-emerald-50' : 'bg-amber-50'} p-3 rounded-lg`}>
                        <p className={`text-xs ${reconciliation.diferenca === 0 ? 'text-emerald-600' : 'text-amber-600'} mb-1`}>Diferença</p>
                        <p className={`text-lg font-bold ${reconciliation.diferenca === 0 ? 'text-emerald-600' : 'text-amber-600'}`}>
                          {formatCurrency(Math.abs(reconciliation.diferenca))}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="mb-4">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-sm font-bold text-blue-600">{reconciliation.percentualConciliacao.toFixed(1)}%</span>
                      </div>
                      <div className="w-full h-2 bg-slate-200 rounded-full overflow-hidden">
                        <div 
                          className="h-full bg-gradient-to-r from-blue-500 to-blue-600 rounded-full"
                          style={{ width: `${reconciliation.percentualConciliacao}%` }}
                        />
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 text-sm text-slate-600">
                      <span>{reconciliation.transacoesConciliadas} conciliadas</span>
                      <span>•</span>
                      <span>{reconciliation.transacoesNaoConciliadas} não conciliadas</span>
                      <span>•</span>
                      <span>{reconciliation.divergencias.length} divergências</span>
                    </div>
                  </div>
