 ...
 * • Filtre por situacao (vencidas, a vencer, pagas)
...
export const ContasPagar: React.FC<ContasPagarProps> = ({ 
  ...
  const [formData, setFormData] = useState<Partial<Transaction>>({
    descricao: '',      // Descricao (ex: "Conta de luz")
    valor: 0,            // Valor (ex: 150.00)
    dataVencimento: '',          // Data de vencimento
    categoria: 'UTILITIES', // Categoria (ex: utilidades)
    situacao: 'PENDENTE',    // Situacao (pago ou pendente)
    formaPagamento: 'PIX', // Forma de pagamento
  ...
  const handleOpenNewTransaction = () => {
    ...
    setFormData({                 // Limpa formulário
      descricao: '',
      valor: 0,
      dataVencimento: new Date().toISOString().split('T')[0], // Hoje
      categoria: 'UTILITIES',
      situacao: 'PENDING',
      formaPagamento: 'PIX',
      observacoes: '',
  ...
  const handleSaveTransaction = async () => {
    // Validação básica
    if (!formData.descricao || !formData.valor || formData.valor <= 0) {
  ...
  const handlePartialPayment = async (transaction: Transaction) => {
    ...
    const valorPago = prompt(
      `Valor total: ${formatCurrency(transaction.valor)}\nDigite o valor que está pagando:`
    );
  ...
  const calcularEncargos = (transaction: Transaction) => {
    ...
    if (transaction.situacao === 'PAID') {
      return { juros: 0, multa: 0, total: transaction.valor, diasAtraso: 0 };
    }
    
    // Calcula dias de atraso
    const hoje = new Date();
    const vencimento = new Date(transaction.dataVencimento || '');
    const diffTime = hoje.getTime() - vencimento.getTime();
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
    const diasAtraso = Math.max(0, diffDays);
    
    // Usa função utilitária para calcular multa
    const multa = calcularMulta(transaction.valor, 2.0); // 2% de multa
    
    // Calcula juros simples (0.33% ao dia)
    const juros = diasAtraso > 0 ? transaction.valor * (0.33/100) * diasAtraso : 0;
    
    // Retorna objeto completo
    return {
      juros: Math.round(juros * 100) / 100,
      multa,
      total: transaction.valor + multa + juros,
      diasAtraso,
    };
  };
          ...
          <tbody className="divide-y divide-gray-200">
            ...
            {isLoading ? (
              ...
              filteredTransactions.map((transaction) => {
                ...
                const situacao = getStatusTransacao(transaction);
                ...
                return (
                  <tr key={transaction.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    {/* Data de Vencimento */}
                    <td className="px-6 py-4">
                      <div className={`flex items-center gap-2 ${situacao === 'VENCIDA' ? 'text-red-600' : 'text-gray-600'}`}>
                        <Calendar size={16} />
                        <span>{formatDate(transaction.dataVencimento || '')}</span>
                        {situacao === 'VENCIDA' && (
                    ...
                    {/* Juros e Multa */}
                    <td className="px-6 py-4">
                      {situacao !== 'PAGA' && encargos.diasAtraso > 0 ? (
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
                        situacao === 'PAGA' ? 'bg-green-100 text-green-800' :
                        situacao === 'VENCIDA' ? 'bg-red-100 text-red-800' :
                        'bg-yellow-100 text-yellow-800'
                      }`}>
                        {situacao === 'PAGA' ? 'Pago' : situacao === 'VENCIDA' ? 'Vencida' : 'A Vencer'}
                      </span>
                    </td>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormData({...formData, formaPagamento: e.target.value as 'PIX' | 'CASH' | 'CREDIT_CARD' | 'TRANSFER'})}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
                >
