 ...
 * ------------------------
 * Componente visual do frontend para recursos humanos.
 *
 * ONDE É USADO?
 * -------------
...
export const RecursosHumanos: React.FC<RecursosHumanosProps> = ({ employees, currentUnitId, evaluations, user }) => {
  ...
  const handleSaveAvaliacao = async () => {
    if (!canWriteHR) {
      alert('Você não tem permissão para registrar avaliações.');
      return;
    }
    ...
    try {
      ...
      await AvaliacaoService.saveEvaluation({
        ...
        overallScore: score,
        overallRating: score >= 90 ? 'EXCELLENT' : score >= 80 ? 'GOOD' : score >= 70 ? 'SATISFACTORY' : 'NEEDS_IMPROVEMENT',
        competencies: [],
        goals: [],
        strengths: null,
        improvements: null,
        actionPlan: avaliacaoForm.observacoes || null,
        status: 'COMPLETED',
        evaluatedBy: null,
      } as any);
      ...
      alert('Erro ao salvar avaliação: ' + (e.message || 'Verifique o console.'));
    }
  };

  const handleSavePDI = async () => {
    if (!canWriteHR) {
      alert('Você não tem permissão para registrar PDIs.');
      return;
    }
    ...
    try {
      ...
      alert('Erro ao salvar PDI: ' + (e.message || 'Verifique o console.'));
    }
  };
                  ...
                  <div className="px-6 pb-6 space-y-4 bg-slate-50/30">
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 p-4 space-y-3">
                      <div className="flex justify-between items-center">
                        ...
                        <button onClick={() => { setSelectedEmployeeId(emp.id); setModalType('avaliacao'); }} disabled={!canWriteHR}
                          className="flex items-center gap-1 text-[10px] font-black text-indigo-600 hover:text-indigo-800 uppercase disabled:opacity-40 disabled:cursor-not-allowed">
                          <Plus size={12}/> Nova
                        </button>
                      </div>
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 p-4 space-y-3">
                      <div className="flex justify-between items-center">
                        ...
                        <button onClick={() => { setSelectedEmployeeId(emp.id); setModalType('pdi'); }} disabled={!canWriteHR}
                          className="flex items-center gap-1 text-[10px] font-black text-indigo-600 hover:text-indigo-800 uppercase disabled:opacity-40 disabled:cursor-not-allowed">
                      ...
                      {rec.pdis.length === 0 ? (
                        ...
                        <div key={pdi.id} className="flex items-center justify-between py-2 border-t border-slate-50">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            }} disabled={!canWriteHR} className="text-[9px] font-black text-slate-400 hover:text-indigo-600 uppercase disabled:opacity-40 disabled:cursor-not-allowed">Avançar</button>
                          </div>
                        </div>
                      ))}
                    </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleSaveAvaliacao} disabled={!canWriteHR} className="flex-1 bg-indigo-600 text-white py-2.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 disabled:opacity-50">
                <Save size={14}/> Salvar
              </button>
            </div>
          </div>
      ...
      {modalType === 'pdi' && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 space-y-4">
            ...
            <div className="flex gap-3">
              <button onClick={() => setModalType(null)} className="flex-1 border border-slate-200 text-slate-600 py-2.5 rounded-xl font-bold text-sm">Cancelar</button>
              <button onClick={handleSavePDI} disabled={!canWriteHR} className="flex-1 bg-indigo-600 text-white py-2.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 disabled:opacity-50">
                <Save size={14}/> Salvar
              </button>
            </div>
          </div>
